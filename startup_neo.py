...
from server.utils import set_httpx_config, api_address
from dialogue_gradio import dialogue_page_gradio


def run_gradio(started_event: mp.Event = None, run_mode: str = None):
    set_httpx_config()
    ...
    def on_click(evt: gr.SelectData, locations, book_name):
        ...
        try:
            ...
            for location, books in locations.items():
                if any(clean_book_name == book.strip() for book in books):
    ...
    with gr.Blocks() as demo:
        ...
        submit_button.click(library_system, inputs=[book_input], outputs=[recommended_book_output, reason_output])
...
async def start_main_server():
    ...
    processes = {
        "online_api_process": None, 
        "openai_api_process": None,
        "model_worker_process": None, 
        "api_process": None, 
        "api_worker_process": None, 
        "webui_process": None
    }
